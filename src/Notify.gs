...
function buildRebalanceMessage_(e) {
  ...
  if (isDD) {
    lines.push('⚠️ DD発動中: TQQQのみ全額キャッシュ化。');
    lines.push('   Gold/Bond (2036/TMF) は常時保有のため売却しません。');
    lines.push('');
  }
  lines.push(
    '📊 実際の保有配分 (Approach A / スリーブ独立):',
    '  TQQQ (NASDAQ 3x):  ' + pct_(holdings.actual_tqqq) + (isDD ? ' ← DDのため0%' : ''),
